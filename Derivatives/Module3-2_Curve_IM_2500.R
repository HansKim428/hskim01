...
## 과거 10년 Rate_IMscn scenario table
Rate_IMscn <- data.frame(DATE = Rate_diff[c((Rate_index - 2499):Rate_index), 1],
                         Call = 0,
                         CD = 0,
                         IRS_6M = 0,
                         IRS_9M = 0,
...
for (j in 1:nrow(Rate_IMscn)) {
    Rate_IMscn[j, c(2:19)] <-
        Rate_diff[match(Rate_IMscn[j, 1], Rate_diff$DATE), c(2:19)] +
        Rate_BDate
}
