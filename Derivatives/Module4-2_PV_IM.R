...
colnames(Result_scn_IM) <- c("Today", paste("scn_", c(1:1250), sep = ""))
...
NPV_sum_pay <- Pos_KRW_FN_IM %>%
    ...
    summarise(across(Today : scn_1250, sum)) %>% 
...
for (i in 1:nrow(NPV_sum_PminusR)) {
    NPV_sum_PminusR[i, 1254] <- sort(as.matrix(NPV_sum_PminusR[i, -c(1, 2)]))[4]
}


IM <- NPV_sum_PminusR %>%
    rename(Min_4th = ...1254) %>%
    mutate(IM = ifelse(Min_4th - Today <= 0, Min_4th - Today, 0)) %>% 
