...
# PT/ND Measure-------------------------------------
System_csv <- data.frame(read.csv('System_procycle.csv'))
...
PT_graph_1Y <-
    ggplot(
        PT_measure %>%
            filter(Maturity %in% c('1Y')) %>%
            select(DATE, Maturity, lamb_97_final),
        aes(x = DATE, y = lamb_97_final, color = 'blue')
    ) +
    geom_col(lwd = 2) +
    ...
    scale_color_discrete(breaks=c('1Y')) +
    labs(title = '장기순응성 추이(1Y)') +
...
PT_graph_20Y <-
    ...
    theme(legend.key.width = unit(1.5, "cm"),
          legend.text = element_text(size = 18)) +
    scale_color_discrete(breaks=c('1Y')) +
    labs(title = '장기순응성 추이(20Y)') +
    scale_x_date(date_breaks = "6 month" , date_labels = "'%y.%b")


PT_graph <- PT_graph_1Y + PT_graph_5Y + PT_graph_10Y + PT_graph_20Y
